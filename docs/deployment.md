...
- 개발 모드 우선순위: `API_BASE_URL` → 과거 차시의 `API_ORIGIN` → `http://localhost:4000`. 30차시부터 로컬을 포함한 production build/start는 명시적인 `API_BASE_URL`이 필수입니다. [장애 재현·복구 기록](incident-30.md)을 참고하세요.
